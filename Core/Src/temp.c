...
static temp_status_t ntc_temp_from_resistance(const resistance_t *resistance,
  ...
  const size_t count = sizeof(ntc_table) / sizeof(ntc_table[0]);
  ...
  if (r <= ntc_table[count - 1].resistance) {
    *temp = ntc_table[count - 1].temp_c;
    return TEMP_ERR_OUT_OF_RANGE;
  }
...
static temp_status_t adc_read_temp_channel(uint32_t *adc) {
  if (adc == NULL) {
    return TEMP_ERR_NULL;
  }
...
static float ntc_resistance_from_adc(uint16_t adc) {
  return TEMP_R_PULLUP_OHM * ((float)adc / (TEMP_ADC_MAX - (float)adc));
}
...
temp_status_t read_temp(temperature_t *temp) {
  ...
  uint16_t adc = adc_read_temp_channel();
  ...
  resistance_t r = ntc_resistance_from_adc(adc);
  temperature_t t = ntc_temp_from_resistance(r);
